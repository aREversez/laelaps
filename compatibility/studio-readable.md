...
| `basic_r4.sdltm`     | 2026-09-25    | Studio 2024 (build 18.0.2.3255) | `e3f3e2d` (fixed writer) | PASS |
| `special_chars_r4.sdltm` | 2026-09-25 | Studio 2024 (build 18.0.2.3255) | `e3f3e2d` (fixed writer) | PASS |
| `numbering_mismatch_r4.sdltm` | 2026-09-25 | Studio 2024 (build 18.0.2.3255) | `e3f3e2d` (fixed writer) | PASS |
| `large_r4.sdltm`     | 2026-09-25    | Studio 2024 (build 18.0.2.3255) | `e3f3e2d` (fixed writer) | PASS |
| `reversed_r4.sdltm`  | 2026-09-25    | Studio 2024 (build 18.0.2.3255) | `e3f3e2d` (fixed writer) | PASS |
| `large_unique_r4.sdltm` | 2026-09-25 | Studio 2024 (build 18.0.2.3255) | `e3f3e2d` (fixed writer) | PASS |

The six `a6580b3` rows are historical results against the pre-fix writer
and stay as-is. `large_v2a/v2b` were round-3 acceptance samples. The six
`*_r4` rows are the **full matrix re-verified against the fixed writer**
(round 4, 2026-09-25) and are the current baseline — every fixture now
passes all four steps, Edit→Commit included, **without any upgrade**.
See the Round-4 section below.
...
### Round-4 (2026-09-25): full matrix re-verification on fixed-writer output — all PASS

All six fixtures regenerated per `fixtures/README.md` recipes with the
round-3 writer (`e3f3e2d`), filenames/TM names suffixed `_r4` to avoid
collisions with the round-1/2 TMs already registered in Studio.

| Fixture | Open | Browse | Search | Edit→Commit (no upgrade) |
|---------|------|--------|--------|--------------------------|
| `basic_r4` | prompt → No, OK | 4 TUs, CJK clean | (phrase not distinctive — see methodology note) | PASS ×1 (after retry) |
| `special_chars_r4` | prompt → No, OK | 2 TUs clean | `R&D` → 1 hit; `esc()` verbatim re-confirmed (`&lt;` literal, `<翻倍>`/`&` intact) | PASS ×1 (after retry) |
| `numbering_mismatch_r4` | prompt → No, OK | 2 TUs | `First sentence` → 1 hit | PASS ×1 |
| `reversed_r4` | prompt → No, OK | 2 TUs, direction `[zh-CN->en-US]` correct | `史密斯` → 1 hit | PASS ×1 |
| `large_r4` (250× dup) | prompt → No, OK | 1000 TUs | `Apple` → exactly 250 hits | PASS — and **`TMTUDuplicate` did NOT reproduce** (see below) |
| `large_unique_r4` | prompt → No, OK | 1000 TUs | `Record 0600` → 1 hit | PASS ×1 |

Every claimed commit was verified by raw-SQLite post-mortem
(`change_user` stamped with the Windows identity, `change_date`
refreshed, per-TU `source_token_data` written; fragment table still
empty — Studio's no-upgrade commit path touches only the edited row).

**Methodology lesson (why "after retry" appears twice)**: the first
batch-A agent run reported basic_r4 and special_chars_r4 edits as PASS
with UI-timestamp evidence, but the DB showed the files byte-unchanged —
false positives caught only by the post-mortem. GUI edit/commit results
must always be confirmed against the SQLite file before being recorded.

**`TMTUDuplicate` correction to round 1's record**: editing one of the
250 identical-segment TUs in `large_r4` and committing now succeeds —
the commit makes that TU's (source, target) unique, so nothing duplicates.
Round 1's `TMTUDuplicate` was therefore path-dependent (it occurred
against the *un-upgraded* schema / when a commit would have produced an
exact existing pair), not an inherent property of duplicate-content TMs.

**Upgrade prompt**: appeared on all six files' first-open-via-dialog,
consistent with round 3's finding that it is unconditional on that path.
