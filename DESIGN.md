...
- XLIFF 1.2/2.0 读写：能打通"译前准备/译后回收"整段工作流的真正 CAT 交换标准，但方言多、工程量大，需要单独立 Phase，不适合顺手做。**已立调研项、未立项开发**，调研立项书见 15.3
- SRX 断句规则导入导出：分句算法是现有核心竞争力之一，开放给高级用户自定义有价值，但要先确认不会削弱现有 `pick_splitter` 的可维护性。**本轮决定暂缓，不排期**，理由与触发条件见 15.3
- LLM 语义 QA：能抓"漏译半句""术语对但语义不对"这类规则 QA 抓不住的问题，但直接冲突 README 的"不联网、不上传文件"承诺——**如果做，必须是显式 opt-in 的可选外挂**（架构上预留 reviewer 接口，本地小模型或用户自备 API key），不能进默认流程，也不能悄悄改变工具箱"纯离线"的定位。**opt-in 骨架已落地**（✅ 2026-09，见 15.3：只有注入点与"默认路径永不触网"的可测试边界，不含任何模型/网络代码）；真实模型集成仍未开始

### 15.3 第三优先批次的启动决策（2026-09）

按"角色高频程度 / 是否离线 / 能否复用现有基础设施"三条标准逐项评估后的启动决定：本轮只动两项（LLM QA 骨架 + XLIFF 调研立项），SRX 挂起。

**LLM 语义 QA：骨架先行（✅ 2026-09），不含任何模型集成**。落地的是"显式 opt-in 外挂"这个架构位置本身，不是功能：

- `language_tools/semantic_review.py` 只定义形状和管道——`Reviewer` 协议（`review(unit) -> list[SemanticIssue]`，逐 unit，不给整批上下文的口子）、`SemanticIssue`（type/description/span_hint，type 不设中央注册表，外部 reviewer 造什么收什么，建议 `SEM_` 前缀防撞名）、`attach()`/`summarize()`。**库内没有也不会有任何默认 reviewer 实现**：reviewer 只能由调用方显式注入——这一条让"默认路径不触网"从"记得保持"的承诺变回结构性事实。不碰凭据：工具箱唯一的持久化通道是 QSettings（Windows 上即注册表明文），API key 不进本项目；真要接本地小模型或远程端点，代码和凭据都在本仓库外的注入方手里。
- 唯一接入点是 `tmtool qa --semantic-review 'module:expr'`（默认不传 = 完全不跑，输出与改动前逐字节一致）；动态 import/求值只存在于 CLI 这层接线，库不背这个责任。命中写进独立通道 `meta['semantic_issues']`，**不并入 `qa_issues`/`qa_confidence`**（同 `term_issues` 独立的理由：外部判断不该稀释规则 QA 的置信度分数），不影响退出码，呈现为"待核实提示"（同 approved 方向的语义定位）。`csv_writer` 的 `include_semantic=True` 加 `semantic_issues` 列；GUI 刻意不给入口——"会离开本机"的操作第一步不能混进"点了必然本机跑完"的现有 QA 按钮，GUI 接线等真实出现第一个可用 reviewer 再说。
- "不联网"升级为可测试不变量：`tests/test_no_network_in_core.py` 用 AST 静态扫描 `language_tools/**/*.py`，出现 `socket`/`requests`/`httpx`/`urllib.request`/`http.client` 任意一种即测试失败（当前全库零命中）。测试见 `tests/test_semantic_review.py`（含 stub 注入的 CLI 端到端）、`tests/test_no_network_in_core.py`。

**XLIFF 1.2/2.0：调研立项，不写实现代码**。TBX 那轮"方言不统一＋私有字段没地方放"的返工教训在先，这一步的产出是决定要不要立项，不是代码：

- **前置问题，先于方言范围回答**：目标用户今天从哪个入口拿到/往哪交回 CAT 交换文件？现有闭环是"双语 docx/xlsx → biconvert → sdltm/TMX 导入 Studio → 成品文件回传"，XLIFF 补的是"把**未翻译**的单语源文件带进去、把译文取回来"这一环——而这一环在 Trados 生态实际常走 SDLXLIFF/项目包而非通用 XLIFF。**若调研结论是现有路径已够用，本项直接取消立项**，不算失败，算省下一座用不上的桥。
- 调研步骤：征集 2-3 份真实 CAT 导出样本（Trados、memoQ 至少各一份；拿不到样本 = 前置问题的答案倾向于取消）→ 盘点顶层结构差异（1.2 与 2.0 是两套不兼容的顶层结构）与私有扩展（`sdl:seg-defs`、`mq:*` 之类）→ 结论落档格式仿 15.2 TBX 一节（"风险栏预判 vs 实际证实情况"）。
